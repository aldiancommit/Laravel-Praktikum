<x-auth-layout title="Register" section_title="Register" section_description="Register using your email">
    <form action="{{ route('auth.store') }}" method="POST" class="flex flex-col gap-4 mt-2">
        ...
        <div class="flex flex-col gap-2">
            ...
            <input type="password" id="confirm_password" name="password_confirmation" class="px-3 py-2 border border-zinc-300 bg-slate-50" placeholder="Confirm your password">
        </div>

        <button type="submit" class="bg-blue-500 border text-white px-3 py-2 text-center gap-2 cursor-pointer mt-4 hover:bg-blue-600 transition-colors">
            <span>Register</span>
        </button>
